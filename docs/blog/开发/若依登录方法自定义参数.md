---
title: 若依登录方法自定义参数
createTime: 2026/06/10 09:23:04
permalink: /blog/tvjmd4r7/
---


## 前言

&emsp;&emsp;若依框架的前后端分离版本中，提供了一个用户名和密码的登录方法，如果我们想增加多参数查询，比如用户类型等，需要修改`UserDetailsServiceImpl.loadUserByUsername(username)`中`username`的含义，比如修改为`json`字符串，含用户名，用户类型等信息，这不符合"开闭原则"思想，于是便有了这篇文章，若依自定义`loadUserByUsername`传参
